...
class GetDefaultNumberController(val context: Context, val view: IDefaultNumberView) : IGetDefaultController {

    var dialog: AlertDialog? = null

    override fun getDefaultNumber() {
...
//        val call: Call<ResponseBody>? = RetrofitHelper.getAPI().getProfile("api/getDefaultNumber?id=" + id + "&token=" + token)
        val call: Call<ResponseBody>? = RetrofitHelper.getAPI().getDefaultNumber(id,token)
        ...
        RetrofitHelper.callApi(call, object : RetrofitHelper.ConnectionCallBack {
            override fun onSuccess(body: Response<ResponseBody?>?) {
                ...
                try {
                    ...
                    val response: GetDefaultNumberResponse = gson.fromJson(reader, GetDefaultNumberResponse::class.java)
                    if (response.status == 1) {
                        /*        Toast.makeText(context, "" + response.getMessage(), Toast.LENGTH_SHORT)
                                    .show()*/
                        view.onResponseDefaultNumber(response.result)
