...
class OtpVerificationController(val context: Context, val view: IOtpVerificationView) : IOtpVerificationController {
    var dialog: AlertDialog? = null

    override fun otpVerification(email: String, otp: String) {
        val call: Call<ResponseBody>? = RetrofitHelper.getAPI().verifyEmailOtp(email, otp)
            ...
            override fun onSuccess(body: Response<ResponseBody?>?) {
                ...
                try {
                    ...
                    if (response.status == 1) {
                        ...
                        view.onOtpVerifyResponse(response)
